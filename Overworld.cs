...
public class Overworld : MonoBehaviour{
    ...
    void Start(){
        for(int i = 0; i < 3; i++){
                Party[i] = Instantiate(PartyFab[i], new Vector3(0 - 2 * i, 0, 0), Quaternion.identity);
                Party[i].name = PartyFab[i].name;}}
    void Update(){
        ...
        foreach(Transform i in Foreground){
            i.position = ((Party[0].position.x - i.position.x) * Party[0].localScale.x > 24) ? new Vector3(i.position.x + 57 * Party[0].localScale.x, 2, 0) : i.position;}
        ...
        if(Party[0].position.x > Distance){
            ...
            if(Distance == 570){
                ...
                for(int i = 0; i < 3; i++){
                    Foreground[i].position = new Vector3(-15 + 19 * i, 2, 0);}
                for(int i = 0; i < 3; i++){FGSpriteRenderer[i].sprite = FGSprites[1];}}}}}